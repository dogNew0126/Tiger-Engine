...
namespace tiger {
	...
	FPSCamera::FPSCamera(const glm::vec3& position = glm::vec3(0.0f, 0.0f, 0.0f), const glm::vec3& up = glm::vec3(0.0f, 1.0f, 0.0f), float yaw = -90.0f, float pitch = 0.0f)
	...
	void FPSCamera::processInput(float deltaTime) {
		...
		// Camera FOV
		float scrollDelta = glm::clamp((float)(InputManager::getScrollYDelta() * 4.0 + (JoystickManager::getButton(0, TIGER_GAMEPAD_A) - JoystickManager::getButton(0, TIGER_GAMEPAD_B) * 2.0)), -4.0f, 4.0f);
