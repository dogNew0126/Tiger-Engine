...
namespace tiger
{
	...
	void PostProcessPass::executePostProcessPass(Framebuffer* framebufferToProcess) {
		...
		m_FxaaShader->setUniform("enableFXAA", m_FxaaEnabled);
		m_FxaaShader->setUniform("showEdges", 0);
		m_FxaaShader->setUniform("read_offset", glm::vec2(1.0f / (float)target->getWidth(), 1.0f / (float)target->getHeight()));
		m_FxaaShader->setUniform("lumaThreshold", 0.5f);
		m_FxaaShader->setUniform("mulReduction", 8.0f);
		m_FxaaShader->setUniform("minReduction", 128.0f);
		m_FxaaShader->setUniform("maxSpan", 8.0f);
